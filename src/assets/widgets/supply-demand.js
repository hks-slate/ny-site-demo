...
window.Widgets['supply-demand'] = function(container) {
  ...
  container.insertAdjacentHTML('beforeend', '<div class="widget-title">Supply &amp; Demand — Interactive</div>');
  ...
  function update(intercept) {
    ...
    eqLineX.attr('x1', xScale(eq.Q)).attr('y1', yScale(eq.P)).attr('x2', xScale(eq.Q)).attr('y2', height);
    eqLineY.attr('x1', 0).attr('y1', yScale(eq.P)).attr('x2', xScale(eq.Q)).attr('y2', yScale(eq.P));
    ...
    eqLabel.attr('x', xScale(eq.Q) + 8).attr('y', yScale(eq.P) - 10).text(`E* (Q=${eq.Q}, P=${eq.P})`);
  }
  ...
  controls.innerHTML = `
    ...
    <input type="range" min="60" max="160" value="100" step="5">
    <output></output>
    <span style="font-size:12px;color:#888;margin-left:8px;">← lower &nbsp;|&nbsp; higher →</span>
  `;
  ...
  slider.addEventListener('input', function() {
    output.textContent = `D-intercept: ${this.value}`;
    update(+this.value);
  });

  output.textContent = `D-intercept: 100`;
  update(100);
};
